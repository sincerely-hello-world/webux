...
> 相比旧链路**去掉了 `386` 与 `armv6`**：两者只在已删除的打包脚本里被半途支持，`386` 也从未进入过发布产物。deb / rpm / pacman 的架构名映射与旧链路输出完全一致（见 10.4 的"架构名映射"）。

**PAM 变体刻意不在 GoReleaser 范围内**：
...
它是 `CGO_ENABLED=1` 且链接目标机的 libpam，无法像其余产物那样静态交叉编译，因此不交给 GoReleaser。**`mise run snapshot` / `mise run release` 产出的包一律是 `CGO_ENABLED=0` 的 shadow 鉴权版本**，PAM 版二进制需单独上传（两个脚本末尾会打印 `gh release upload …` 的提示）。这也顺带解决了旧链路里两个脚本自称产出 `webux-pam-linux-amd64`、而打包脚本却去 `webux-linux-<arch>` 查找的这个始终对不上的问题。
...
**产物清单**（输出目录 `build/dist/`，共 **16** 个发布产物）：

| 类别            | 命名                              | 数量 | 说明                                      |
| --------------- | --------------------------------- | ---- | ----------------------------------------- |
| 安装树 tar.gz   | `webux_<ver>_linux_<arch>.tar.gz` | 3    | **推荐主产物**：解压即得到完整目录树        |
| 纯二进制 tar.gz | `webux_<ver>_linux_<arch>_bin.tar.gz` | 3 | 归档里只有一个 `webux`，无目录前缀        |
| deb             | `webux_<ver>_<arch>.deb`          | 3    | `<arch>` ∈ amd64 / arm64 / armhf          |
| rpm             | `webux-<ver>-1.<arch>.rpm`        | 3    | `<arch>` ∈ x86_64 / aarch64 / armv7hl     |
| pacman          | `webux-<ver>-1-<arch>.pkg.tar.zst` | 3   | `<arch>` ∈ x86_64 / aarch64 / armv7h      |
| 校验和          | `webux_<ver>_checksums.txt`       | 1    | sha256，覆盖上面 15 个                     |
...
>
> `build/dist/` 里除了上述 16 个发布产物，还会有 goreleaser 自己的 `metadata.json` / `artifacts.json` / `config.yaml`，以及按目标命名的中间二进制目录（`webux_linux_amd64_v1` 等）—— 它们不会被发布，但会出现在目录列表里。
...
| 校验和          | `webux_<ver>_checksums.txt`           | 覆盖上面 15 个                                               |
...
mise run uninstall       # 卸载，保留 /var/lib/webux
```
...
它会依次：要求 root → 识别架构（amd64 / arm64 / armv7）与发行版 → 识别 init 系统（systemd / openrc / sysv，并据此生成对应服务定义）→ 从 `https://github.com/${WEBUX_REPO:-brendan4linux/webux}/releases/download/<tag>/webux_<ver>_linux_<arch>.tar.gz` 下载并解压 → 安装二进制与配置（已存在的配置不覆盖）→ 启动服务。

> ⚠️ **该脚本依赖“安装树”归档（不带 `_bin` 后缀）**，因为它是从归档里的 `etc/webux/config.yaml` 取配置模板。下载 `webux_<ver>_linux_<arch>_bin.tar.gz`（纯二进制）会因找不到模板而失败。
>
> 相比旧版本：资产名与 GoReleaser 的命名精确对齐（脚本内部用 `ASSET_VERSION="${VERSION#v}"` 把 `v0.1.0` 还原成 `0.1.0`），旧写法 `webux-linux-<arch>.tar.gz` 会 404；架构识别已改为与发布产物一致的 `amd64 / arm64 / armv7`（不再接受 `armv6` / `386`）。
...
mise run snapshot
# 3. 核对产物数量（应恰好 16 个发布产物 = 3 deb + 3 rpm + 3 zst + 3 安装树 + 3 二进制 + 1 校验和）
#    注意 build/dist/ 里还有 goreleaser 自己的 metadata.json / artifacts.json / config.yaml
#    以及按目标命名的中间二进制目录，所以不要用 `ls | wc -l` 计数
ls -1 build/dist | grep -c '\.deb$'                                 # 3
ls -1 build/dist | grep -c '\.rpm$'                                 # 3
ls -1 build/dist | grep -c '\.zst$'                                 # 3
ls -1 build/dist | grep -cE '_linux_(amd64|arm64|armv7)\.tar\.gz$'  # 3（安装树）
ls -1 build/dist | grep -c '_bin\.tar\.gz$'                        # 3（纯二进制）
# 4. 抽查包元数据与校验和
...
> **实测基线（2026-09，GoReleaser 2.18.1 / Go 1.25）**：`goreleaser release --snapshot --clean` 约 **9–14s** 完成（旧链路的打包阶段约 50s），产出上述 16 个文件；`sha256sum -c` **15/15** 通过。PAM 变体需另外单独构建与上传。
> 注意中文 locale 下 `sha256sum -c` 的输出是“OK: 15”／“成功”，不是 `: OK`，脚本里不要按英文关键字 grep。
...
mise run snapshot                               # 本地试跑：全部产物 → build/dist/（16 个文件）
...
> 本项目的所有开发约定（工具链版本、nub 迁移步骤、.npmrc 陷阱、dev 运行方式）同时记录在仓库记忆 `webux-tooling` 中，供持续维护。
